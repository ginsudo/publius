...
- **`constitutional_section`** — ID reference (or array of ID references) into the Constitution corpus, identifying the constitutional provisions this item argues about, observes, or interprets. Type is `string[] | null`. Values are Constitution corpus item IDs of the form `"constitution:<locator>"` — e.g., `["constitution:art1.sec8.cl3"]` for the Commerce Clause, `["constitution:amdt14.sec1.cl4"]` for the Equal Protection Clause. A single item may reference multiple provisions: `["constitution:art1.sec8.cl3", "constitution:art1.sec8.cl18"]`. The two states distinguish editorial status:
  - **`null`** — the item has not yet been editorially reviewed for constitutional cross-references. This is the default state on parse. Used to audit which items still need editorial pass before retrieval can rely on this field.
  - **`[]`** — the item has been editorially reviewed and no constitutional provision is asserted (Tocqueville items observing constitutional culture without addressing a specific provision; Federalist papers concerned with topics other than constitutional structure).
  - **Non-empty array** — reviewed, with one or more provisions asserted.

  Universal field, but coverage is uneven by corpus: Federalist and SCOTUS items are organized around constitutional structure and are expected to populate densely; Tocqueville is organized by observed pattern, not by provision, and reviewed-with-`[]` will be the common state. On Constitution corpus items themselves, this field is always `[]` — the Constitution is the referent, not a referrer, and the "unreviewed" state does not apply. Do not use this field as a cross-corpus filter or retrieval key until after editorial review of every corpus is complete (see DECISIONS.md).
