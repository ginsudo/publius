...
**Chunk text format conventions:** Header lines are kind-aware per corpus. Federalist's header is `Federalist No. {N} — {title}\nAuthor: {author(s)}`. Tocqueville's header is `Tocqueville, Democracy in America — {locator}\nAuthor: Tocqueville`, where the locator is `Volume {Roman}, Part {N}, Chapter {N}: {French title}` for chapters, `Volume {Roman}, End-Note {letter}{ (page N) if available}` for end-notes, `Volume {Roman}, Introduction` (or `Part II Preamble`) for the two introductions, and `Volume {Roman}, Avertissement (Tenth Edition)` for the avertissement. The French chapter title appears in the header even though the body of the chunk is English (the editorial-gated `translation` field). Voyage-4-large handles French; the title is the canonical content signature.
