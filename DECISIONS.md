...
## Prompt extraction convention: `## The prompt` → first `---`, single function

**Decision:** Every prompt file in this repo follows one convention. The model-facing prompt text lives between a `## The prompt` heading (optionally with a version suffix like `(v0.1)`) and the first `---` horizontal rule on its own line after that heading. Everything outside that span — purpose, register notes, design rationale, workflow framing, failure-mode predictions, terminology decisions, anything else — is human-only documentation and is never sent to the model. One extraction function reads this convention: `extractPrompt(filePath)` in `lib/ask.ts`. Any script or route that loads a system prompt imports `extractPrompt` from `lib/ask.ts`; no script implements its own prompt loader.

**Reasoning:** Before this entry there were two conventions in the tree. The Q&A pipeline (`prompts/system-prompt-v0.2.md`, loaded by `/api/ask` and the harness) used `## The prompt` → first `---`. The translation prompt (`prompts/tocqueville-translation-system.md`, loaded by `scripts/generate-translation.ts`) used `---` → end-of-file, with a local `loadSystemPrompt()` helper. Two conventions multiplies failure modes for no benefit: a reader can't predict where the prompt body lives from the filename alone; an extraction bug in either helper goes unobserved by the other; the `prompt_sha256` audit point in annotations files becomes harder to reason about across corpora. The Q&A convention won — it had an explicit heading (signposted, greppable), survived the v0.1 → v0.2 rewrite, and was already battle-tested by the prompt-sha256-pinning machinery. The translation prompt was restructured to match in a refactor commit; the prompt content was unchanged, only the file's organization moved.
...
**Validation gate result — FAILED (May 2026).** The v2 back-test against the 378 Volume I records produced 1 case of \`classifier=accept × owner=flagged_for_rewrite\`. Numerically 0.26% — near zero. But the cause is qualitatively bad: the classifier hallucinated the rendering content (asserted "rendering shows corrected form" for a \`Hecwelder → Heckewelder\` spelling standardization when the rendering still reads \`Hecwelder\`). This is rendering misreading, not borderline judgment. v2 is not validated. The full Federalist triage remains deferred until the rendering-misreading failure mode is mitigated; the next-version mitigation candidates (verbatim-quoting instruction, regex pre-check for spelling standardizations, or other) are an owner decision, deliberately not patched reactively. Confusion matrix and full case analysis in IMPLEMENTATION_LOG.md.
...
## Multi-corpus retrieval index: one file, shared schema, append-only builders (May 2026)

**Decision:** All corpora share a single SQLite file at `data/eval/index.sqlite`. The `chunks` and `chunks_vec` schemas are corpus-agnostic and are not extended per corpus. Each corpus has its own append-only build script (`build-index.ts` for Federalist, `build-tocqueville-index.ts` for Tocqueville Vol I, and one per future corpus). Federalist's builder is the only one that drops and recreates tables; every subsequent corpus's builder asserts the schema exists, refuses to overwrite by default, and inserts at `MAX(rowid)+1`. The `chunks.corpus` column is the discriminator at retrieval time.

**Why one file:** Cross-corpus probes (P14 / P15) require a single `MATCH` query to return both corpora. The vec0 virtual table is corpus-agnostic by rowid; the metadata column `chunks.corpus` carries the discriminator. Bundle size on Vercel is well under the function-bundle ceiling (25 MB at Federalist + Tocqueville Vol I, ceiling 250 MB), and `outputFileTracingIncludes` already references the path. Two-file alternatives require query-time fan-out for no gain.

**Why no per-corpus columns:** Federalist's `paper_number` is the only existing Federalist-specific column on `chunks`; it is nullable, and Tocqueville rows store `paper_number: null`. Tocqueville's structural metadata (volume / part / chapter / item_kind) is encoded in `item_id` (`tocqueville:vol1.part2.ch7`, `tocqueville:vol1.t1.notes.A`) and is derivable. No consumer filters on these fields at the retrieval layer today; if one ever needs to (e.g., "give me Vol I Part 2 hits only"), `ALTER TABLE` then — don't pre-build for it. SCOTUS will face the same question at Phase 6: its precedent is to encode structural metadata in `item_id` first.

**Tocqueville per-row conventions:** `paper_number: null`, `authors_json: '["Tocqueville"]'`, `authorship_status: 'undisputed'`. The "disputed authorship" concept is Federalist-specific; `'undisputed'` keeps the Hit type's `authorship_status: string` honest without inventing a new sentinel value. Future SCOTUS rows will reuse `authors_json` for the opinion's author; `authorship_status: 'undisputed'` is the right default (opinions don't have disputed authorship in the Federalist sense).

**Append-only build discipline:** A corpus's builder must (a) assert `chunks` and `chunks_vec` exist with the expected columns, (b) refuse to run if its own corpus's rows already exist unless `--force` is passed (which deletes the matching rows from both tables atomically before re-embedding), (c) read `MAX(rowid)` and insert starting at the next free row, (d) never touch other corpora's rows. Federalist's builder is the exception — it is the schema's source of truth and drops/recreates tables on every run because it has no prior corpus to preserve.

**Chunk text format conventions:** Header lines are kind-aware per corpus. Federalist's header is `Federalist No. {N} — {title}\nAuthor: {author(s)}`. Tocqueville's header is `Tocqueville, Democracy in America — {locator}\nAuthor: Tocqueville`, where the locator is `Volume {Roman}, Part {N}, Chapter {N}: {English title}` for chapters, `Volume {Roman}, End-Note {letter}{ (page N) if available}` for end-notes, `Volume {Roman}, Introduction` (or `Part II Preamble`) for the two introductions, and `Volume {Roman}, Avertissement (Tenth Edition)` for the avertissement. The English chapter title (`item.tocqueville.translated_title`, with fallback to `item.title` if null) matches the English body translation; the title is the canonical content signature. (The header originally carried the French `item.title`; switched to English in the May 2026 re-embed for retrieval alignment — see the dedicated entry below for the full rationale.)

**Probe set runner:** `data/eval/run.ts` defaults to running only `phase_5_only: false` probes (13 of 16 at present). The new `--phase5` / `--include-phase5` flag runs all probes; required to exercise the cross-corpus P14 / P15 / P16 probes added at probe-set authoring time. The default remains the 13-probe Federalist set so the Phase 1.1 sign-off baseline stays stable across re-runs.

**Known artifact, not silently fixed:** `data/eval/probes.json` P16's `must_include` carries `"kind": "end_note"`. The runner's `chunkMatches()` compares this against the chunk-level `kind` column (`'body'` or `'footnote'`), not the item-level Tocqueville kind. The result is a false-negative `must_include` report on P16 even when the end-note item dominates the top-10. Item_id alone uniquely pins the end-note item; dropping the `kind` qualifier from P16 closes the gap. Held for owner editorial decision on the probe set; not patched in the embedding slice.

---

## Tocqueville chunk header: English title (May 2026 re-embed)

**Change applied:** As of the May 2026 re-embed, the Tocqueville chunk header's chapter locator uses `item.tocqueville.translated_title` (with fallback to `item.title` if null) instead of the French `item.title`. The same field swap also applies to the `chunks.title` metadata column for both body and footnote rows. Implemented at the source in `data/eval/lib.ts` (lines 227, 290, 318); re-embedded via `build-tocqueville-index.ts --force`, 3680 chunks, 384k tokens. Federalist rows untouched.

**Rationale:** The display layer was updated to show English titles throughout (browse list, reading view h1, citations panel). English titles are more useful for the retrieval surface — user queries arrive in English, and embedding English-language chapter titles improves chunk-to-query alignment for title-targeted queries. Fixing at the source plus a `--force` re-embed avoided an API-side display-layer hack and kept the chunk text and `chunks.title` metadata consistent with the reading-view display.

**Field:** `item.tocqueville.translated_title`, fallback to `item.title` if null.
