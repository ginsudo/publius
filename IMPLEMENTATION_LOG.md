...
Phase 1.2 is the Q&A layer: a Claude API call that takes a question, retrieves with `queryIndex()`, and returns an answered text. The system prompt is the central artifact. As of this writing, the prompt is locked at v0.1, the test harness is built, an A/B run and a five-clause ablation against the closing section have been executed against a 15-question test set, and v0.2 has not yet been decided.

**Locked artifacts:**
- `prompts/system-prompt.md` — v0.1 of the Q&A system prompt. The file mixes a human-only preface (design notes, predicted failure modes, open questions) with the model-facing prompt itself. The structural-convention note near the top defines the boundary: the prompt sent to the model is the section between the `## The prompt` heading and the next `---` rule. Everything outside that span never reaches the model. The full v0.1 entry is in `DECISIONS.md`.
- `prompts/test-questions.md` — 15-question test set across 5 categories (best-student-tier, mediocre-but-not-trivial, out-of-corpus, genuine disagreement, capability-matrix probes). Each question carries an italics `*What it tests:*` line that the harness strips before sending to the model.
...
- No v0.2 system prompt. v0.1 stays locked at `prompts/system-prompt.md` until the result files have been read and a successor is decided.
