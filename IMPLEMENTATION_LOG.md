...
- *Same retrieval set as runC.* Q11 returned the same 10 documents as runC, slightly reordered (top-2 swapped, scores ~0.02 lower across the board). Q7 returned 8/10 of runC's hits (missing fed:50 ¶8 disputed, fed:82 ¶1, fed:40 footnote 2, fed:84 ¶9; gained fed:81 ¶5, fed:39 ¶15, fed:39 ¶7, fed:85 ¶15) with scores systematically ~0.14 lower. To isolate whether the Q7 drift was route-introduced or embedding-side, ran the same Q7 question through the harness directly: harness returned the same hits as the route within sub-0.01 float noise (0.343 vs 0.340, 0.332 vs 0.329, etc.). The route is structurally equivalent to the harness; the runC drift is voyage embedding non-determinism between morning and afternoon, not route-introduced. Worth carrying forward as a watch item — voyage embeddings are not bit-stable across calls — but does not affect Q&A discipline since the retrieval set is qualitatively the same and the model handles ranking variation gracefully.
